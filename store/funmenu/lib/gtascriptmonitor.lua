...
scriptmanager = menu.list(miscmenu, GTA_MONITOR, {}, "")
scriptmenu = {}

util.create_tick_handler(function()
	for keys, gtascript in ipairs(gtascriptname) do
		if SCRIPT.DOES_SCRIPT_EXIST(gtascript) then
			if SCRIPT.HAS_SCRIPT_LOADED(gtascript) and (scriptmenu[keys] == nil)then
				scriptvalue = menu.list(scriptmanager, gtascript, {}, "")
				scriptmenu[keys] = scriptvalue
				...
				while not (scriptiterator == 0) do
					if (SCRIPT._GET_NAME_OF_THREAD(scriptiterator) == gtascript) then
						dividername = "Thread ID : "..scriptiterator
					end
					scriptiterator = SCRIPT.SCRIPT_THREAD_ITERATOR_GET_NEXT_THREAD_ID()
				end
				menu.divider(scriptvalue, dividername)
			end
		end
	end
	for delscript = 1, 990 do
		if not SCRIPT.HAS_SCRIPT_LOADED(gtascriptname[delscript]) and not (scriptmenu[delscript] == nil) then
			pcall(menu.delete, scriptmenu[delscript])
			scriptmenu[delscript] = nil
		end
		util.yield(0)
	end
end)